...
export default async function EditPage({ params }) {
  ...
  if (url && key) {
    ...
    const { data } = await supabase
      .from('opportunities')
      .select('id, title, summary, deadline, event_start_date, event_end_date, recurrence, age_from, age_to, cost_type, opportunity_type, format, cities, countries, is_international, source, source_url, apply_url, status, price_note, details, featured_week')
      .eq('id', params.id)
      .maybeSingle();
    opp = data;
  }
