...
async function sendPayOffer(bot, sub, chatId, supabase) {
  ...
  //
  // Переліку «Що входить» тут теж більше немає (Марія, 24.09.2026: «який сенс
  // на кнопці оплати показувати, що входить у неї»). Людина дійшла сюди через
  // /plus і всю анкету — вона вже знає, за що платить. Опис лишається там, де
  // його читають до рішення: на сторінці /plus.
...
async function continueStart(bot, supabase, sub, chatId, handle) {
  ...
  // Анкета вже є — ведемо одразу до телефону й оплати. Проміжний екран
  // «Ваша анкета. Все правильно?» звідси прибрано 24.09.2026 (Марія:
  // «прибери цей крок»): він стояв між людиною й оплатою щоразу, коли вона
  // поверталась у бота. Подивитись і переробити анкету нікуди не зникло —
  // це /profile і кнопка «✏️ Змінити анкету» під самою пропозицією оплати.
  else await afterProfile(bot, supabase, sub, chatId);
}

// Показати анкету на вимогу: команда /profile до оплати. У дорозі до оплати
// цього екрана більше немає (24.09.2026) — він там лише додавав зайве
// натискання. Але побачити й переробити анкету, не маючи підписки, лишається
// можливим: меню й /form відкриваються тільки підписникам, тож без цього
// людина до оплати не могла б навіть глянути, що про дитину записано.
