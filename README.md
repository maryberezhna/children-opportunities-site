...
### Платформа можливостей для українських дітей 0–18 років — в Україні та за кордоном
...
```
┌──────────────────────────┐         ┌─────────────────────┐         ┌──────────────────────┐
│ 🐍 Скрапери й агент      │ ──────▶ │ 🗄️  Supabase        │ ──REST─▶│ 🌐  Next.js           │
│ scraper/ (цей репо)      │         │ Postgres 17         │         │ App Router + RSC     │
│ реєстр джерел у базі     │         │ opportunities       │         │ ISR + ревалідація    │
└──────────────────────────┘         │ sources, raw_items  │         │ Hosted on Vercel     │
             │                       └─────────────────────┘         └──────────────────────┘
             ▼                                  ▲                                ▲
┌──────────────────────────┐                    │                                │
│ 🧱 raw_items             │ ───LLM-екстракція──┘                                │
│ сира знахідка + хеш      │    ворота: лінк, 5 полів, дублі                     │
└──────────────────────────┘                                                     │
                                                                ┌────────────────┴───────┐
                                                                │ 👥 Люди                │
                                                                │ Батьки та діти         │
...
Пайплайн один. Скрапери, агент пошуку й пропозиції відвідувачів — лише постачальники: усе знайдене спершу лягає в `raw_items`, а далі йде спільними воротами (живий лінк, п'ять обов'язкових полів, дедуплікація, три коридори модерації).

- **🐍 Python** (`scraper/` у цьому репо) — скрапери, LLM-екстракція, дедуплікація, планові перевірки. Список джерел живе в таблиці `sources`: вимкнути джерело — це `UPDATE`, без деплою.
- **🟨 Node-пайплайн видалено 20.09.2026.** Він писав у базу напряму, в обхід обов'язкових полів, і з розкладу його зняли ще 15.09. За весь час дав 10 записів, останній — у травні. Записи лишились, джерела позначені в реєстрі як вимкнені.
