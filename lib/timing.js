...
export function isExpired(item, todayIso) {
  ...
  const results = daysUntil(item?.results_date, todayIso);
  if (dl !== null && dl < 0) return true;
  if (end !== null && end < 0) return true;
  if (dl === null && end === null && start !== null && start < 0) return true;
  // Лише дата результатів (розіграш, оголошення переможців) — після неї кінець.
  if (dl === null && end === null && start === null && results !== null && results < 0) return true;
  return false;
}
 ...
 *   { state: 'results', days, date } — лише дата розіграшу / оголошення переможців
...
export function whenState(item, todayIso) {
  ...
  if (results !== null && results >= 0) {
    return { state: 'results', days: results, date: item.results_date };
  }
...
export function whenRank(item, todayIso) {
  ...
  if (['deadline', 'event', 'running', 'results'].includes(s.state)) return s.days;
  return Number.POSITIVE_INFINITY;
}

// Три найближчі дедлайни — живий доказ у блоці «Кава чи можливість» на
// головній. Минулі дати не беремо: прострочений дедлайн у закличному блоці
// гірший за відсутній блок.
export function soonestDeadlines(opportunities, todayIso, limit = 3) {
