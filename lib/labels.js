...
// Платне називаємо платним. «Доступно» на платній програмі — підміна: родина
// бачить на сторінці дедлайнів чи в превʼю посилання м'яке слово й
// дізнається про оргвнесок лише на сайті організатора.
...
// Заповнена дата проведення — пряма ознака датованої події, хоч би який тип.
// Початок і кінець рівноцінні: у записі може бути відомий лише один із них.
export const isEvent = (item) =>
  EVENT_TYPES.has(item?.opportunity_type)
  || Boolean(item?.event_start_date)
  || Boolean(item?.event_end_date);
...
export function formatLabel(raw, lang = 'uk') {
  ...
  // in-person і «в-очне» — це теж наживо, але регулярка їх не ловила, і
  // картки лишались без підпису. «заочн» свідомо не додаємо: воно містить
  // «очн», а означає протилежне.
  const offline = /офлайн|offline|наживо|in-person|в-очн/.test(v);
...
export const NEED_LABELS_EN = {
  ...
  autism: 'autism',
  idp: 'displaced',
  veteran_family: 'children of defenders',
