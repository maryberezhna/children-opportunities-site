// Діалогова форма профілю дитини для Telegram-бота (age → interests → cost).
//
// Стать дитини питали раніше, але нею ніде не користувались: у доборі
// можливостей поля gender немає жодного разу. Питання коштувало зайвого
// кроку у воронці й ламало обіцянку «ми не питаємо дитячих даних».
...
export async function beginFlow(bot, supabase, chatId, handle) {
  ...
  await bot.sendMessage(chatId, '🧡 <b>Налаштуймо добірку</b>\n\nТри короткі питання — і ми надсилатимемо можливості саме для вашої дитини. Імені дитини й школи не питаємо: лише вік, інтереси й чи показувати платне.');
  await bot.sendMessage(chatId, Q_AGE, multiKb('age', FLOW_AGE, []));
}

export async function finishFlow(bot, chatId) {
  await bot.sendMessage(chatId,
    '🎉 <b>Профіль готово!</b>\n\n• ⚡ надішлемо можливість, щойно вона зʼявиться, — саме під вашу дитину\n• ⏰ нагадаємо про дедлайн за 7 і 2 дні\n• 📝 допоможемо із заявкою — просто напишіть сюди\n\nЗмінити відповіді — /start · Відписатися — /stop');
}
...
export async function handleFlowCallback(bot, supabase, cbq) {
  ...
  if (!sub) { await bot.answerCallback(cbq.id, 'Почніть з /start'); return; }
  ...
  if (step === 'age' || step === 'int') {
    ...
    if (val === '__done') {
      if (!(sub[col] || []).length) { await bot.answerCallback(cbq.id, step === 'age' ? 'Оберіть хоча б один вік' : 'Оберіть хоча б один інтерес'); return; }
  ...
  // Хтось міг лишитись на вже неіснуючому кроці «стать» у момент викату —
  // не лишаємо його в глухому куті, а ведемо далі до питання про вартість.
  if (step === 'gender') {
    await save({ flow_step: 'cost' });
    await bot.answerCallback(cbq.id);
    await bot.sendMessage(chatId, '💳 <b>Показувати платні можливості чи лише безкоштовні?</b>', singleKb('cost', FLOW_COST, 2));
