...
const Q = {
  // Перше питання анкети батьки з кількома дітьми читають як питання про всіх,
  // тож без префікса прямо кажемо: позначайте всі діапазони. Для другої й
  // наступних дітей (префікс «Дитина 2. ») питання лишається про одну
  // дитину (Марія, 19.09.2026).
  age: (p) => (p
    ? `👶 <b>${p}Скільки років дитині?</b>\nОберіть один або кілька діапазонів, тоді «Далі».`
    : '👶 <b>Скільки років дитині або дітям?</b>\nОберіть один або кілька діапазонів, тоді «Далі». Якщо дітей кілька — позначте всі, що підходять.'),
...
export async function saveCustomCity(bot, supabase, chatId, sub, text) {
  ...
  await bot.sendMessage(chatId,
    ...
    + (found
      ...
      : 'Щойно там зʼявиться можливість — надішлемо. А поки надсилатимемо те, що проходить по всій Україні й онлайн.')
