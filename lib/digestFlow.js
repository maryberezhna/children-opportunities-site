...
const Q = {
  ...
  place: `📍 <b>Де шукати можливості?</b>\n${PLACE_ALWAYS} Місто лише додає те, що поруч: оберіть одне чи кілька, тоді «Далі». Підходить будь-де — просто «Далі».\nНемає вашого міста — натисніть «Мого міста немає» і впишіть його.`,
...
export async function handleFlowCallback(bot, supabase, cbq) {
  ...
  if (step === 'place') {
    ...
    if (val === '__done') {
      ...
      await bot.editMessage(chatId, mid, `📍 Де: <b>${esc(placeSummary(sub.places))}</b> ✅`);
