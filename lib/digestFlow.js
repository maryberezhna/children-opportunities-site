...
export async function beginFlow(bot, supabase, chatId, handle) {
  ...
  await bot.sendMessage(chatId, '🧡 <b>Налаштуймо підбірку</b>\n\nВідповідай на кілька питань — і надсилатимемо можливості саме для твоєї дитини. Ми не запитуємо дитячих даних — лише вік та інтереси.');
  await bot.sendMessage(chatId, Q_AGE, multiKb('age', FLOW_AGE, []));
}

export async function finishFlow(bot, chatId) {
  await bot.sendMessage(chatId,
    '🎉 <b>Профіль готово!</b>\n\n• ⚡ надішлемо можливість щойно вона зʼявиться — саме під твою дитину\n• 📝 допоможемо з поданням заявки — просто напиши сюди\n• 🎁 подарунки від партнерів\n\nЗмінити відповіді — /start · Відписатись — /stop');
}
