...
/** Дистанційний формат: нікуди їхати не треба. */
export const isOnline = (o) =>
  /онлайн|online|дистанц/i.test(o.format || '')
  || (o.cities || []).some((c) => /онлайн|online/i.test(c));

export const realCities = (o) =>
  (o.cities || []).filter((c) => !PSEUDO_CITIES.has(String(c).toLowerCase().trim()));

export const abroadCountries = (o) =>
  (o.countries || []).filter((c) => String(c).toLowerCase() !== 'ua');

/** Чи є в записі конкретне українське місто: подія фізично відбувається там.
 *  «Вся Україна», «Онлайн» і «Міжнародні» — не місця, а підписи. */
const inUkrainianCity = (o) =>
  realCities(o).length > 0 && (o.countries || []).some((c) => String(c).toLowerCase() === 'ua');

/** Чи дитина фізично їде за межі України. */
export function goesAbroad(o) {
  if (abroadCountries(o).length) return true;
  // Подія в конкретному українському місті за кордон нікого не везе, хай
  // яким міжнародним її вважає модель. 23.09.2026 картка English Club із
  // Києва («спілкування з американцями») писала «За кордоном», бо прапорець
  // is_international перебивав місто: модель ставила його за «учасники з
  // різних країн». Місто конкретніше за прапорець — воно й виграє.
  if (inUkrainianCity(o)) return false;
