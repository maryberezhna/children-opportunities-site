 ...
 *   • waitingReason(row) — чому запис чекає машину, а не людину: бракує поля
 *     або цитати. Такий запис лишається чернеткою, на сайт не йде і в
...
export function waitingReason(row = {}) {
  ...
  if (missing.length) return `бракує: ${missing.join(', ')}`;
  const noProof = missingProof(row);
  if (noProof.length) return `без цитати: ${noProof.join(', ')}`;
  return null;
}
 ...
 */
export function splitQueue(drafts = []) {
  const forHuman = [];
  const waiting = [];
