...
// partially_free і subsidized база ще приймає, але опублікувати запис із ними
// не можна: батько не дізнається, чи доведеться платити.
const COST_TYPES = new Set([
  'free', 'paid_affordable', 'paid_premium',
]);
...
// пропусти термін, якщо неможливо знайти»): громади платять за програмою на
// роки вперед, і сторінка ради часто не каже про строк подання. Дзеркало
// PAYMENT_TYPES у scraper/normalizer.py.
const PAYMENT_TYPES = new Set(['allowance', 'support_payment']);
...
export function missingRequired(row = {}) {
  ...
  // Дата проведення закриває вимогу нарівні з дедлайном: подія з відомими
  // «6–8 листопада» і без оголошеної подачі — повноцінний запис.
  if (!row.deadline && !row.event_start_date && !row.event_end_date
      && !row.results_date && !row.recurrence
      && !PAYMENT_TYPES.has(row.opportunity_type)) missing.push(DATE);
