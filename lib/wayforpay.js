...
export function invoiceBody(sub, plan = 'monthly', { early = false, now = new Date() } = {}) {
  const yearly = plan === 'yearly';
  const regularAmount = yearly ? AMOUNT_YEAR : AMOUNT;
  const discounted = early && !yearly;
  const amount = discounted ? AMOUNT_EARLY : regularAmount;
  const product = yearly ? 'Підписка Dityam+ (рік)'
