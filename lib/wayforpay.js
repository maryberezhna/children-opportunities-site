...
//      WAYFORPAY_AMOUNT (грн/міс), WAYFORPAY_AMOUNT_YEAR (грн/рік),
//      WAYFORPAY_AMOUNT_EARLY (перший місяць для списку очікування), SITE_URL.
...
// Список очікування (рішення Марії 16.09.2026): перший місяць безкоштовний.
// WayForPay не проводить платіж на 0 грн — регулярна підписка починається з
// реального списання, тож перший місяць коштує 1 грн, далі повна ціна.
const AMOUNT_EARLY = Number(process.env.WAYFORPAY_AMOUNT_EARLY || 1);
...
export const PRICE_EARLY = AMOUNT_EARLY;
...
//
// early — знижка для списку очікування: ПЕРШИЙ місячний платіж PRICE_EARLY, а
// регулярні списання — повна ціна. За документацією WayForPay regularAmount —
// «Amount of regular payment. If not transferred, the amount is taken from the
// 'amount' field», а dateNext — дата першого регулярного списання
// (https://wiki.wayforpay.com/en/view/852102). Тобто без явного regularAmount
// людину щомісяця списували б по 1 грн назавжди. На річну знижка не діє.
// firstAmount — ціна ПЕРШОГО платежу за промокодом (lib/promo.js). На відміну
// від early, діє й на річний план: код `first` дає перший рік за 499 замість
// 999, а поновлення через рік — знову повна ціна, бо regularAmount не чіпаємо.
export function invoiceBody(sub, plan = 'monthly', { early = false, firstAmount = null, now = new Date() } = {}) {
  ...
  const discounted = promo !== null || (early && !yearly);
  const amount = promo !== null ? promo : (discounted ? AMOUNT_EARLY : regularAmount);
