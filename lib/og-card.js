...
export function ogCard({ opportunities, sources }) {
  return h('div', {
    ...
    h('div', { key: 'body', style: { display: 'flex', flexDirection: 'column', gap: 20 } }, [
      ...
      // «Кожну перевіряємо вручну» прибрано 14.09.2026: це неправда, записи
      // проходять автоматичні перевірки. «Доступні» замість «платні» — теж.
      }, 'Безкоштовні й платні програми для дітей 0–18 років в Україні й за кордоном. Кожна веде на офіційне джерело.'),
    ]),

    h('div', { key: 'stats', style: { display: 'flex', gap: 56 } }, [
      stat(String(opportunities), 'активних можливостей'),
