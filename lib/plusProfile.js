...
const PLACE_PSEUDO = new Set([PLACE_ONLINE, PLACE_ABROAD, PLACE_OTHER]);
...
export function chosenCities(places) {
  return (places || []).filter((p) => !PLACE_PSEUDO.has(p));
}

/**
 * «Де» для підсумку профілю. З 22.09.2026 онлайн, всеукраїнське й закордонне
 * приходить кожному, тож показуємо лише міста (або «будь-де»), а старі
 * значення online/abroad зі збережених профілів — не показуємо як вибір.
 */
export function placeSummary(places) {
  const cities = chosenCities(places);
  const always = 'усе онлайн, всеукраїнське й закордонне';
  if (cities.length) return `${cities.join(', ')} + ${always}`;
  if ((places || []).includes(PLACE_OTHER)) return `${always}, без міста`;
  return 'будь-де';
}

// Місто, вписане вручну. Кнопок у боті вісім — міста, де записів найбільше, а
// родині з Ніжина чи Коломиї не було що натиснути: «Мого міста немає» лише
// розширювало добір на всеукраїнське й ніде не памʼятало саме її місто.
//
// Назву не вигадуємо й не виправляємо по суті: прибираємо «м.», зайві пробіли
// й регістр («КИЇВ» і «київ» → «Київ»), бо збіг із полем cities шукається
// точний. Чого нема в базі — лишаємо як написали: бот одразу чесно скаже, що
// можливостей там поки немає.
export const MAX_CUSTOM_CITIES = 3;
const CITY_MAX_LEN = 30;
const CITY_SHAPE = /^[\p{L}][\p{L}\s'ʼ’.-]*$/u;

export function parseCustomCities(text) {
  const src = String(text || '');
  // Посилання й пошта розпадаються скісною на «слова», схожі на місто
  // («example.com/misto» → «Misto»), тож такий текст відкидаємо цілком.
  if (/https?:|www\.|@/i.test(src)) return [];
  const out = [];
  const seen = new Set();
  for (const raw of src.split(/[,;\n/]+/)) {
    const s = raw.trim().replace(/\s+/g, ' ').replace(/^(?:м\.\s*|місто\s+)/i, '').trim();
    if (s.length < 3 || s.length > CITY_MAX_LEN || !CITY_SHAPE.test(s)) continue;
    const city = s.toLowerCase().replace(/(^|[\s-])(\p{Ll})/gu, (_, sep, ch) => sep + ch.toUpperCase());
    const key = city.toLowerCase();
    if (seen.has(key)) continue;
    seen.add(key);
    out.push(city);
    if (out.length === MAX_CUSTOM_CITIES) break;
  }
  return out;
}
...
/**
 * Чи підходить місце. Онлайн, всеукраїнське й закордонне підходить кожному
 * завжди. Місто лише додає офлайн-записи, де це місто є в cities. Порожній
 * вибір — будь-де. Запис без жодної позначки місця при обраному місті НЕ
 * підходить: вгадувати, де він, ми не будемо.
 *
...
export function placeOk(o, places) {
  ...
  if (o.format === 'online' || o.format === 'hybrid' || cities.includes('Онлайн')) return true;
  if (cities.includes('Вся Україна')) return true;
  if (o.is_international
      || (o.countries || []).some((c) => c && c !== 'ua')
      || cities.includes('Міжнародні')) return true;
  return chosenCities(places).some((c) => cities.includes(c));
}
