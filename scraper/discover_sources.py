...
# Профіль «фонди й організації» (discover-rare.yml, рішення Марії 14.09.2026):
# хто системно робить рідкісні програми для дітей з України — фонди відомих
# людей і клубів, міжнародні дитячі фонди, культурні інститути.
PROFILE = (os.environ.get("DISCOVER_SOURCES_PROFILE") or "").strip()

# Фонди цікаві тоді, коли їхніми програмами може скористатися дитина, яка живе
# в Україні, — а не лише діаспора, що вже в країні (уточнення Марії 14.09.2026).
FOUNDATIONS_FOCUS = """

ОКРЕМА УМОВА ДЛЯ ЦЬОГО ПОШУКУ: шукай організації, чиїми програмами може
скористатися дитина, яка ЖИВЕ В УКРАЇНІ — вони запрошують чи привозять дітей з
України або приймають заявки з будь-якої країни. Організації, що допомагають
лише дітям-біженцям, які вже живуть у своїй країні (місцеві гуртки, шкільна
інтеграція, доплати за адресою), не додавай.
"""

FOUNDATION_THEMES = [
    "благодійні фонди відомих українців — спортсменів, музикантів, акторів — з програмами для дітей",
    "фонди спортивних клубів і федерацій, що запрошують дітей з України на табори й турніри",
    "міжнародні дитячі фонди з окремими програмами для дітей з України",
    "культурні інститути й посольства з програмами для українських школярів",
    "університети й наукові центри з літніми програмами для школярів з-за кордону",
    "фонди, що організовують безкоштовний відпочинок і реабілітацію дітей з України за кордоном",
    "мистецькі й музичні фонди зі стипендіями чи майстер-класами для дітей з України",
]
