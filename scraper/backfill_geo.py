                    ...
                    "properties": {
                        ...
                        "is_international": {
                            ...
                            "description": "Міжнародного рівня: закордонний "
                                           ...
                                           "true. FALSE: гурток чи школа в "
                                           "українському місті (навіть якщо "
                                           "вихованці їздять на міжнародні "
                                           "фестивалі) та відбірковий етап в "
                                           "Україні, якщо в назві "
                                           "«Всеукраїнська» чи «Український».",
                        },
                    },
...
# Скільки країн має сенс тримати в одному записі. Для «Європейського корпусу
# солідарності» модель чесно перелічила всі 26 країн ЄС — формально правда,
# практично сміття: підпис на картці все одно каже «За кордоном», а майбутній
# фільтр «Де» від такого запису отримав би збіг із будь-якою країною. Коли
# країн більше за цю межу, конкретної географії в можливості просто немає —
# лишаємо порожньо, а «за кордон» виводиться з is_international.
MAX_COUNTRIES = 5
...
def _clean(codes) -> list[str]:
    ...
    for c in codes or []:
        ...
        if code in VALID_COUNTRIES and code not in out:
            out.append(code)
    return [] if len(out) > MAX_COUNTRIES else out
