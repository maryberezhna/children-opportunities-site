...
_REPEATS = re.compile(
    ...
    # Римський номер — лише ВЕЛИКИМИ літерами («VII Всеукраїнський»,
    # «XII всеукраїнська»). Без цього IGNORECASE читав сполучник «і» як
    # римську «І»: «заняття і відкриті для всіх» ставало «щорічним»
    # (21.09.2026, арттерапія в Запоріжжі).
    r"|(?-i:\b[IVXLCІХ]{1,7})\s+(?:всеукраїнськ|міжнародн|регіональн|обласн|міськ|відкрит)"
...
PERMANENT_RECHECK_DAYS = 90      # постійна: не рідше раз на 3 місяці (Марія, 21.09.2026)
...
def permanent_recheck_days(row: dict) -> int:
    ...
    90 днів — для тих, де постійність прочитана в тексті: найдовший інтервал,
    який Марія допускає («раз в 3 місяці найдовше», 21.09.2026). Для здогаду
