    ...
    "input_schema": {
        ...
        "properties": {
            "deadline": {"type": "string",
                         "description": "YYYY-MM-DD — ЛИШЕ останній день ПОДАЧІ заявки, якщо він прямо названий. Дата проведення події — НЕ дедлайн."},
...
1. deadline — ЛИШЕ якщо в тексті прямо названий останній день ПОДАЧІ заявок.
   Дата проведення події (табору, фестивалю, фіналу) — НЕ дедлайн, її сюди не
   став. Формат YYYY-MM-DD. Рік не вказано — найближчий майбутній. НЕ вигадуй.
...
def main() -> int:
    ...
    rows = (client.table("opportunities")
            ...
            .is_("deadline", "null")
            .is_("recurrence", "null")
            # Запис із датами проведення вже має час життя. Раніше цей скрипт
            # брав і такі, і міг вписати початок події в deadline поверх
            # правильно розкладених дат.
