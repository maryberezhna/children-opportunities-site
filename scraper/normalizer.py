...
# Учасник — дорослий, а дитина лише умова участі. 21.09.2026 на сайті висіла
# «Безкоштовна бізнес-програма для батьків із можливістю отримати грант на
# власну справу» з віком 0–18: модель прочитала «батьки сімей із дітьми до 18
# років» як вік учасника. Поміряно на базі: правило ловить рівно два записи
# (ту програму й «Грантова програма для жінок „СТВОРЮЙ!“») і жодного зайвого.
# Виплати й допомога на дитину сюди не потрапляють — вони для родини заради
# дитини.
_ADULT_AUDIENCE = re.compile(
    r"(?:для|батькам|мамам|жінкам)\s+(?:батьк|мам|матер|жінок|підприєм|ветеран|вчител|педагог|дорослих|освітян)",
    re.IGNORECASE)
_ADULT_ACTIVITY = re.compile(
    r"бізнес|підприєм|власн\w* справ|працевлашт|перекваліф|кар.єр\w* (?:для|батьк|жінок)"
    r"|стартап\w* для (?:батьк|жінок|дорослих)|грант\w* на (?:власн|свою|відкриття)",
    re.IGNORECASE)
_FOR_THE_CHILD_TYPES = {"allowance", "support_payment", "humanitarian", "medical_aid",
                        "shelter", "legal_aid"}
...
def _sanitize(data: dict, source_text: str = "") -> dict:
    ...
    if looks_adult_participant(data):
        data["status"] = "draft"
...
МІСЦЕВА ПОСЛУГА ЧУЖОЇ КРАЇНИ — НЕ НАША, навіть якщо формально відкрита для
всіх. Муніципальний фонд одного міста, гурток при тамтешній бібліотеці,
клуб вихідного дня в британському містечку, оплата секцій від голландської
громади — усе це для місцевих мешканців, а не для нашої аудиторії. Став
confidence нижче 0.5, щоб запис пішов людині. Беремо з-за кордону лише:
(а) те, що прямо адресоване дітям з України, біженцям чи діаспорі;
(б) те, на що дитина може податися з України: обміни, стипендії, конкурси,
табори з набором іззовні, онлайн-курси.

ЧУЖІ ВИПЛАТИ Й СУБСИДІЇ — НЕ НАШІ (Марія, 23.09.2026). Дитяча допомога,
соціальна виплата, субсидія чи компенсація від іншої держави, її громади або
місцевого фонду призначена тим, хто там живе, і родина в Україні нею не
скористається. Конкурси, обміни й стипендії на навчання за кордоном — це не
виплати: вони лишаються. Українські виплати й пільги збираємо як раніше.
...
МОВА: title, summary і details — ЗАВЖДИ українською, навіть коли джерело
англійською чи іншою мовою: перекладай, нічого не додаючи. Власну назву
програми чи організації (WWOOF, Erasmus+, Yale Young Global Scholars) лишай в
оригіналі й додавай через тире коротке українське пояснення з тексту:
«WWOOF — волонтерство на органічних фермах». Міста й країни — українською.
...
- Можливість лише для тих, кому БІЛЬШЕ 18 (нижня межа 19, 21, 25…), або
  лише для студентів і випускників університетів (магістратура, PhD).
  ⚠️ «18+» — НЕ причина відхиляти: 18-річні — наша аудиторія
  (рішення Марії 21.09.2026). European Solidarity Corps (18–30),
  DiscoverEU для 18-річних, волонтерські табори «від 18» — ПРИЙМАЙ,
  age_from=18, age_to=18.
- Можливість, де УЧАСНИК — ДОРОСЛИЙ, а діти лише умова участі: «бізнес-
  програма для батьків сімей із дітьми до 18 років», «грант для жінок-
  підприємиць», «працевлаштування для мам», «курс для вчителів». Навчається,
  відкриває справу чи отримує грант на власний бізнес дорослий — це не
  можливість для дитини, хай скільки разів у тексті є слово «діти».
  ⚠️ Виплати й допомога НА ДИТИНУ (виплата родинам з дітьми, оздоровлення,
  гуманітарна допомога дітям) — це для родини ЗАРАДИ дитини, їх ПРИЙМАЙ.
...
- Якщо нижня межа віку вища за 18 або потрібен диплом університету →
  confidence=0.1. Програма «з 18 років» — звичайна, НЕ знижуй за неї
  confidence.
...
EXTRACT_TOOL = {
    ...
    "input_schema": {
        ...
        "properties": {
            "title": {"type": "string",
                      "description": "Українською. Власна назва програми — в оригіналі, "
                                     "з українським поясненням через тире."},
            "summary": {"type": "string", "maxLength": 400,
                        "description": "Українською, навіть якщо джерело іншою мовою."},
...
# Модель інколи віддає українську латиницею: 22.09.2026 на сайт вийшов запис
# із коротким описом «Matematychnyy konkurs-hra "Giochi d'Autunno" dlya
# shkolyariv Italiyi». Промпт вимагає українську, але не гарантує її, тож
# перевіряємо самі: таке не відхиляємо (текст по суті правильний), а віддаємо
# людині — переписати кирилицею.
def latin_fields(data: dict) -> list:
    """Поля, де українського тексту немає: серед літер менше половини кирилиці.

    Назву судимо суворіше за описи. Правило Марії (23.09.2026): на сайті все
    українською, а власна назва лишається лише з поясненням через тире —
    «English Sky — мовна школа англійської». Тож назва без жодної кириличної
    літери («Bisons», «Hello English») теж іде людині, хоч вона й коротка.
    """
    bad = []
    for key in ("title", "summary", "details"):
        ...
        if key == "title":
            if cyr == 0 and len(letters) >= 3:
                bad.append(key)
            continue
        if len(letters) < 40:            # уривок чи абревіатура — не судимо
            continue
...
Витягни дані через extract_opportunity."""
            ...
            # Українською — або до людини. Див. latin_fields вище.
            latin = latin_fields(data)
            if latin:
                data["status"] = "draft"
                note = f"переписати українською: {', '.join(latin)} латиницею"
                data["admin_comment"] = ((data.get("admin_comment") or "") + " · " + note).strip(" ·")
